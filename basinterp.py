        ...
        while 1:
            ...
            if op == 'END':
                ...
                if portvar == "CONSOLE":
                    ...
                    if not self.vars.has_key("SPIR0W1"):
                        self.vars["SPIR0W1"] = 0 #0 means read, 1 means write
                    if not self.vars.has_key("SPIMCPADDR"):  
                        self.vars["SPIMCPADDR"] = 0 
                    if not self.vars.has_key("SPIREADDATA"):  
                        self.vars["SPIREADDATA"] = 88 
                    if not self.vars.has_key("SPICS"):  
                        self.vars["SPICS"] = 0 
                    if not self.vars.has_key("SPIRESET"):  
                        self.vars["SPIRESET"] = 0 
                        
                    opcode = ((self.vars["SPICS"]&0x07)<<1)|0x40  

                    if(self.vars["SPIRESET"]): 
                        self.cali.mcp_reset() 
                        self.vars["SPIRESET"] = 0

                    if(self.vars["SPIR0W1"]): # write
                        self.cali.mcp_spi_write(opcode, self.vars["SPIMCPADDR"],int(cmd)) 
                    else:
                        self.vars["SPIREADDATA"] = self.cali.mcp_spi_read(opcode, self.vars["SPIMCPADDR"])  
                elif portvar == "SPIDAC":
                    if not self.vars.has_key("SPIR0W1"):
                        self.vars["SPIR0W1"] = 1 #0 means read, 1 means write
                    if not self.vars.has_key("SPIDACADDR"):  
                        self.vars["SPIDACADDR"] = 1 #DATA register
                    if not self.vars.has_key("SPIREADDATA"):  
                        self.vars["SPIREADDATA"] = 0 
                    if not self.vars.has_key("SPICS"):  
                        self.vars["SPICS"] = 3 
                        
                    if(self.vars["SPIR0W1"]): # write                     
                        #fct_di = (int((int(cmd)* 4096 * 16)/24) & 0xFFF0) # (i)  *(2^12/(24-0))<<4    # select 0~24mA
                        self.cali.dac_spi_write(self.vars["SPICS"], self.vars["SPIDACADDR"],int(cmd)) 
                    else:
                        readback = self.cali.dac_spi_readback(self.vars["SPICS"], int(cmd)) 
                        self.vars["SPIREADDATA"] = round((readback & 0xFFF0)*24.0/(16.0*4096))
                else:
#                    self.cali.set_console_text(port + " " + str(cmd))
                    # todo: Need check new UUTBAUD is compatible with multi port pactch
                    ...
                    # if not self.vars.has_key("UUTBAUD"):
                    #     self.vars["HMIBAUD"] = 38400               

                    self.cali.ack_to_plying = 1
                    # if portvar == "MCUPORT":
                    #     uart_conn_result = self.cali.set_uart_text(port, self.vars['MCUBAUD'], str(cmd), self.vars['CHECK'])
                    # elif portvar == "HMIPORT":
                    #     uart_conn_result = self.cali.set_uart_text(port, self.vars['HMIBAUD'], cmd, self.vars['CHECK'])
                    # else:
                    if port in self.cali.ser:
                        baudrate = self.cali.ser[port][1]
                        uart_conn_result = self.cali.set_uart_text(port, baudrate, str(cmd))
                ...
                if not self.vars.has_key("ENPRINT"):
                    self.vars["ENPRINT"] = 0  
                           
                if (portvar == "FCT") :
                    self.cali.start_fct(self.vars['HMIPORT'], self.vars['SCADAPORT'], self.vars['UART0PORT'], self.vars["HMIBAUD"], cmd, self.vars["ENPRINT"])

                while True:
                    import fct_ccb                            
                    if self.cali.fct_ccb_running == 0:
                        break
                    time.sleep(0.01)
            elif op == 'DELAY':
